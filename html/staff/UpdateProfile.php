                ...
                <div id="experienceList">
                    <?php foreach($exps as $exp): ?>
                        <div class="experience-card">
                            <h4>
                                <?= $exp['year_start'] ?> - 
                                <?= $exp['year_end'] ? $exp['year_end'] : "Hiện tại" ?> :
<?= htmlspecialchars($exp['title']) ?>
    ...
    <script>
        // ✅ FIX: Dùng array đã fetch sẵn
let currentSkills = <?= json_encode(array_column($skills, 'skill_name')) ?>;
        let currentExperiences = <?= json_encode($exps) ?>;
